...
*Setup instructions for a gitian build of CrystalPro using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CrystalPro
...
- [Building CrystalPro](#building-crystalpro)
...
Clone the git repositories for crystalpro and gitian and then checkout the crystalpro version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the crystalpro repository
...
To build CrystalPro (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the crystalpro repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/crystalpro/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/crystalpro/crystalpro
...
For example:
```bash
URL=https://github.com/konez2k/crystalpro.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit crystalpro=${COMMIT} --url crystalpro=${URL} ../crystalpro/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit crystalpro=${COMMIT} --url crystalpro=${URL} ../crystalpro/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit crystalpro=${COMMIT} --url crystalpro=${URL} ../crystalpro/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/crystalpro-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/crystalpro-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/crystalpro-build.assert
```
...
[crystalpro/gitian.sigs](https://github.com/crystalpro/gitian.sigs/) repository, or if that's not possible to create a pull
request.
