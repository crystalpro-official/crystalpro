...
#if defined(NDEBUG)
#error "CrystalPro cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ucrp) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight == 1){nSubsidy = 19000000 * COIN;}
    else if (nHeight > 1 && nHeight <= 200)  { nSubsidy = 5000 * COIN;}
...
{
    RenameThread("crystalpro-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CRP: We use certain sporks during IBD, so check to see if they are
