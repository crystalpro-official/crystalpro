...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CRP address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no crystalpro: URI
...
{
    // Convert crystalpro:// to crystalpro:
    //
    //    Cannot handle this later, because crystalpro:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open crystalpro.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CrystalPro.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CrystalPro.lnk
...
{
    return GetAutostartDir() / "crystalpro.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a crystalpro.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CrystalPro\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the crystalpro app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add crystalpro app to startup item list
