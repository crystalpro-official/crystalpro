...
#define NEWS_URL "https://rfc.crystalpro.org/category/news/feed/"
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CRP)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount crpAvailableBalance = balance - immatureBalance - nLockedBalance;

    // CRP Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // CRP labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, crpAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // CRP Available
    bool showCRPAvailable = settingShowAllBalances || crpAvailableBalance != nTotalBalance;
    bool showWatchOnlyCRPAvailable = showCRPAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showCRPAvailable || showWatchOnlyCRPAvailable);
    ui->labelBalance->setVisible(showCRPAvailable || showWatchOnlyCRPAvailable);
    ui->labelWatchAvailable->setVisible(showCRPAvailable && showWatchOnly);

    // CRP Pending
    bool showCRPPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyCRPPending = showCRPPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showCRPPending || showWatchOnlyCRPPending);
    ui->labelUnconfirmed->setVisible(showCRPPending || showWatchOnlyCRPPending);
    ui->labelWatchPending->setVisible(showCRPPending && showWatchOnly);
    ...
    // CRP Locked
    bool showCRPLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyCRPLocked = showCRPLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showCRPLocked || showWatchOnlyCRPLocked);
    ui->labelLockedBalance->setVisible(showCRPLocked || showWatchOnlyCRPLocked);
    ui->labelWatchLocked->setVisible(showCRPLocked && showWatchOnly);
